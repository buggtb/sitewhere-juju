...
INSTALL_PACKAGES = ['mongodb-org=3.0.4', 'python-yaml']
...
INIT_CHECK_DELAY = 1.5

# number of times mongo_client_smart will try to execute given statement
MONGO_CLIENT_RETRIES = 10
...
def update_file(filename=None, new_data=None, old_data=None):
    juju_log("update_file: %s" % filename, level=DEBUG)
...
def join_replset(master_node=None, host=None):
    # TODO: This methoud shouldn't accept any arguments becase we don't care
    #       about master_node - we always run replset admin commands on
    #       localhost.
